...
test('loadPgn - works - comments', () => {
  ...
  const comments = [
    {
      fen: 'rn1qkbnr/ppp2ppp/3p4/4p3/3PP1b1/5N2/PPP2PPP/rnbqkbnr w KQkq - 1 4',
...
test('loadPgn - works - comments (before first move)', () => {
  ...
  const comments = [
    ...
    {
      fen: 'rnbqkbnr/pppp1ppp/4pn2/8/2PP4/6P1/PP2PP1P/RNBQKBNR b KQkq - 0 3',
      comment:
...
test('loadPgn - works - move annotations (including check symbols)', () => {
  const chess = new Chess()
  const fen = 'rnb2q1r/pp1k2pp/2pb1n2/8/3P4/5N2/PPP2PPP/rnbqkbnr w KQ - 0 8'
  const pgn =
